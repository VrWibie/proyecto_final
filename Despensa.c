...
//función para traer archivo de texto al ejecutable(Hipertensión)
...
FILE *file= fopen("Hipertension.txt", "r");// 'r' para abrir lectura de archivo Hipertension
    ...
    while(fgets(texto, sizeof(texto), file)){ //sizeof calcula el tamaño en bytes de su operenado
...
//función para traer archivo de texto al ejecutable(Diabetes)
...
//función para taer archivo de texto al ejecutable (Obesidad)
...
//función para traer archivo de texto al ejecutable (sin enfermedad)
